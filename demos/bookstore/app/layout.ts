...
export const { Document$ } = component(
  'Document',
  ({ title = 'Bookstore', children }: { title?: string; children?: Remix.RemixNode }) =>
    html(
      { lang: 'en' },
      head$(
        meta({ charSet: 'UTF-8' }),
        ...
        title$(title),
        script({ type: 'module', async: true, src: routes.assets.href({ path: 'entry.js' }) }),
        style({
          innerHTML: `
          /* CSS Reset */
...
export const { Layout$ } = component('Layout', ({ children }: { children?: Remix.RemixNode }) => {
  ...
  return Document$(
    header$(
      div(
        { class: 'container' },
        h1$(a({ href: routes.home.href() }, '📚 Bookstore')),
        nav$(
          a({ href: routes.home.href() }, 'Home'),
          a({ href: routes.books.index.href() }, 'Books'),
          a({ href: routes.about.href() }, 'About'),
          a({ href: routes.contact.index.href() }, 'Contact'),
          a({ href: routes.cart.index.href() }, 'Cart'),
          user
            ? Fragment$(
                a({ href: routes.account.index.href() }, 'Account'),
                user.role === 'admin' ? a({ href: routes.admin.index.href() }, 'Admin') : null,
                form(
                  {
                    method: 'POST',
            ...
            : Fragment$(
                a({ href: routes.auth.login.index.href() }, 'Login'),
                a({ href: routes.auth.register.index.href() }, 'Register'),
              ),
        ),
      ),
    ),
    main$(div({ class: 'container' }, children)),
    footer$(
      div(
        { class: 'container' },
        p$(`© ${new Date().getFullYear()} Bookstore Demo. Built with Remix.`),
      ),
    ),
  )
})
