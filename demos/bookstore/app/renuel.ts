import { Fragment as RemixFragment, Frame as RemixFrame } from '@remix-run/dom'
import { createElement } from '../node_modules/@remix-run/dom/dist/lib/component.js'
import type { HTMLElements } from '../node_modules/@remix-run/dom/dist/lib/html-types.js'
import type { Remix } from '@remix-run/dom'

type RemixAttributes = {
  key?: Remix.Key
}

type ComponentProps<T> = T extends keyof HTMLElements ? Remix.Props<T> : Remix.ComponentProps<T>
...
/**
 * Props excluding children and including global Remix attributes
 * @typeParam Props - The set of props from which to extract attribute props
 */
type AttributeProps<Props> = SafeOmit<Props, 'children'> & RemixAttributes
...
type ChildrenArgs<Props> = Props extends { children?: infer Children }
  ? (
      Children extends string ? string : Children extends Iterable<infer Item> ? Item[] : Children
    ) extends infer C
    ? C extends unknown[]
      ? C
...
type NoExcessProps<ExpectedProps, ActualProps> = ExpectedProps & {
  [P in keyof ActualProps]: P extends keyof ExpectedProps ? ExpectedProps[P] : never
}
 ...
 */
type StandardFactory<Props> = <const P>(
 ...
 * @typeParam Props - The type of props `ElementType` accepts
 */
type SkipPropsFactory<Props> =
  AnyRequired<AttributeProps<Props>> extends false
    ? (...children: ChildrenArgs<Props>) => Remix.RemixElement
    : never

/**
 * A curried function that accepts remaining props and returns a Remix element
 * @typeParam Props - The remaining props that the function accepts
 */
type CurriedFinalFunction<Props> =
  AnyRequired<Props> extends true
    ? <const P>(props: NoExcessProps<Props, P>) => Remix.RemixElement
    : <const P>(props?: NoExcessProps<Props, P>) => Remix.RemixElement
 ...
 */
type PartialFactory<Props, ElementType extends Remix.ElementType> = <const P0>(
  props: NoExcessProps<Partial<AttributeProps<Props>>, P0>,
  ...children: ChildrenArgs<Props>
) => SafeOmit<AttributeProps<Props>, keyof P0> extends infer P1 ? CurriedFinalFunction<P1> : never
 ...
 * @typeParam ElementType - The type of element the factory produces
 */
type PartialSkipPropsFactory<Props, ElementType extends Remix.ElementType> = (
  ...children: ChildrenArgs<Props>
) => CurriedFinalFunction<AttributeProps<Props>>
...
export const component = <
  ...
  ElementType extends Remix.ElementType,
  Props = ComponentProps<ElementType> & DataProps<ElementType>,
>(
  name: Name,
  elementType: ElementType,
): RemoveNever<
  Record<Name, StandardFactory<Props>> &
    Record<`${Name}$`, SkipPropsFactory<Props>> &
...
> => {
  ...
  let skipProps = (...children: Parameters<typeof standard>[1][]) => standard({}, ...children)

  let partial =
    (p0: Parameters<typeof standard>[0], ...children: Parameters<typeof standard>[1][]) =>
    (p1: Parameters<typeof standard>[0]) =>
      standard({ ...p0, ...p1 }, ...children)

  let partialSkipProps =
...
/** Fragment factories */
export const { Fragment, Fragment$ } = component('Fragment', RemixFragment)

/** Frame factories */
export const { Frame } = component('Frame', RemixFrame)

/** Factory helper for HTML/SVG tags */
const tag = <TagName extends keyof HTMLElements>(tagName: TagName) => component(tagName, tagName)
...
export const { blockquote, blockquote$, _blockquote, _blockquote$ } = tag('blockquote')
...
export const { figcaption, figcaption$, _figcaption, _figcaption$ } = tag('figcaption')
...
export const { var: var_, var$: var_$, _var: _var_, _var$: _var_$ } = tag('var')
